export default {
  ...
  mounted() {
    ...
    googletag.cmd.push(() => {
      ...
      if(article !== null) {
        var cats = article.cats || [];
        var catString = ',';
        for(var i=0;i<cats.length;i++){
          var name = cats[i].name.trim();
          if(name === '') continue;
          catString += cats[i].name + ',';
        }
        adSlot.setTargeting('category', catString);
      }
      ...
      if(article !== null) {
        ...
        for(var i=0;i<tags.length;i++){
          var name = tags[i].name.trim();
          if(name === '') continue;
          tagString += name + ',';
        }
        adSlot.setTargeting('tag', tagString);
      }
