export default {
  ...
  mounted() {
    ...
    googletag.cmd.push(() => {
      ...
      if (page !== null && page.post_id != null) {
          adSlot.setTargeting('post_id', page.post_id);
      } else if(article !== null && article.id != null) {
        adSlot.setTargeting('post_id', article.id);
      }
      ...
      // set targeting page url
      if (page !== null ) {
        adSlot.setTargeting('page_url', page.url);
        googletag.pubads().set('page_url', page.url);
      }
      ...
      // set targeting url
      adSlot.setTargeting('url', '');
      adSlot.setTargeting('path', '');
      adSlot.setTargeting('fullpath', '');
      if(route !== null) {
        adSlot.setTargeting('url', route.path || '');
        adSlot.setTargeting('path', route.path || '');
        adSlot.setTargeting('fullpath', route.path || '');
        if(route.query) {
          for(var objKey in route.query) {
            adSlot.setTargeting('query_' + objKey, route.query[objKey]);
          }
        }
      }
      ...
      if(article !== null) {
        ...
        adSlot.setTargeting('category_slug', categoryToString(categoriesMap, article.cats, 'slug'));
      }
...
function categoryToString(catsMap = {},arr = [], attr = 'name') {
  let returnVal = ',';
  for(let i=0; i<arr.length; i++){
    const cat = arr[i]
    const parent = catsMap[cat.id]
    if(!!parent) {
      returnVal += parent[attr] + ',';
    } 
    returnVal += cat[attr] + ',';
  }
