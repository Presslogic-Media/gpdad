export default {
  ...
  mounted() {
    ...
    googletag.cmd.push(() => {
      ...
      if (subDomain !== null) {
          adSlot.setTargeting('subdomain', subDomain);
      }
      // set targeting post_id
      if (postId !== null) {
          adSlot.setTargeting('post_id', postId);
      }
      // set targeting env
      if (env !== null) {
          adSlot.setTargeting('env', env);
      }
      // set targeting inskin_desktop_yes
      // if (inskinDesktopYes !== null) {
      //     adSlot.setTargeting('inskin_desktop_yes', inskinDesktopYes);
      // }
      let inskin_desktop_yes = "false";
      adSlot.setTargeting('inskin_desktop_yes', inskin_desktop_yes);
      if(screenWidth != null) {
        adSlot.setTargeting('inskin_desktop_yes', screenWidth >= 1345 ? 'true' : 'false');
      }

      // set targeting inskin_mobile_yes
      // if (inskinMobileYes !== null) {
      //     adSlot.setTargeting('inskin_mobile_yes', inskinMobileYes);
      // }
      let inskin_mobile_yes = "false";
      ...
      if (adDemoKey !== null) {
          adSlot.setTargeting(adDemoKey, adDemoVal);
      }

      adSlot.setTargeting('category', category);
