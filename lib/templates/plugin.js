...
export default async function (ctx, inject, env) {
  ...
  const inAd = `
      ...
      if (interstitialSlot) {
          ...
          googletag.pubads().set('page_url', '${env.BASE_URL}');
          interstitialSlot.setTargeting('subdomain', '${env.DEFAULT_PAGE}').addService(googletag.pubads());
      }
  `;
  gptInitScriptHtml += `
