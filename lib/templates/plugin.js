...
export default async function (ctx, inject) {
  ...
  const interAdId = '/123517519/presslogic-web-interstitial';
  const interAdEvent = 'slotOnload'
  const inAd = `
      interstitialSlot = googletag.defineOutOfPageSlot(${interAdId},googletag.enums.OutOfPageFormat.INTERSTITIAL);
      if (interstitialSlot) {
        interstitialSlot.addService(googletag.pubads());
    
        googletag.pubads().addEventListener(${interAdEvent}, function(event) {
        console.log(interstitialSlot.getSlotId().getId())
        console.log(event.slot.getSlotId().getId())
        if (interstitialSlot === event.slot) {
            console.log(${interAdEvent})
          }
        });
      }
  `;
